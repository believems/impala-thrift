...
func (p *TTypeNode) Equals(other *TTypeNode) bool {
	...
	if !p.ScalarType.Equals(other.ScalarType) {
		return false
	}
	if len(p.StructFields) != len(other.StructFields) {
		return false
	}
...
func (p *TAggregateFunction) Equals(other *TAggregateFunction) bool {
	...
	if !p.IntermediateType.Equals(other.IntermediateType) {
		return false
	}
	if p.IsAnalyticOnlyFn != other.IsAnalyticOnlyFn {
		return false
	}
	if p.UpdateFnSymbol != other.UpdateFnSymbol {
		return false
	}
	if p.InitFnSymbol != other.InitFnSymbol {
		return false
	}
...
func (p *TFunction) ReadField3(ctx context.Context, iprot thrift.TProtocol) error {
	...
	for i := 0; i < size; i++ {
		...
		p.ArgTypes = append(p.ArgTypes, _elem4)
	}
	if err := iprot.ReadListEnd(ctx); err != nil {
		return thrift.PrependError("error reading list end: ", err)
	}
...
func (p *TFunction) Equals(other *TFunction) bool {
	...
	for i, _tgt := range p.ArgTypes {
		_src5 := other.ArgTypes[i]
		if !_tgt.Equals(_src5) {
			return false
		}
	}
	if !p.RetType.Equals(other.RetType) {
		return false
	}
	...
	if !p.ScalarFn.Equals(other.ScalarFn) {
		return false
	}
	if !p.AggregateFn.Equals(other.AggregateFn) {
		return false
	}
